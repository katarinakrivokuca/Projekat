...
\\
\\
\indent Ako uzmemo potprostor dimenzije $k+1$  generisan istim $k$ potprostorom kojim je generisana traka, i proizvoljnim vektorom sa sfere, projekcija $k$-trake na njega bi\' ce ba\v s traka u potprostoru $\mathbb{R}^{k+1}$ , \v cime je dokaz kompletan.
$\square$
\end{proof}
\\
...
\end{proof}
\\
\begin{df} Geometrijska medijana skupa ta\v caka $S=\{ X_1, X_2, ..., X_m\}$, gde su $X_i\in \mathbb{R}^n$ je ta\v cka $M\in \mathbb{R}^n$ za kogu va\v zi:
